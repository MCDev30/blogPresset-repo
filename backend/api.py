...
def user_register():
    ...
    data, query = register(username, password, email,
                           access_token, "Inscription effectuée avec succès")
...
def set_user_active(email, active_status):
    ...
    cursor.execute("UPDATE User SET active = %s WHERE email = %s",
                   (active_status, email))
    cursor.execute("UPDATE User SET code = %s WHERE email = %s",
                   (str(random.randint(111111, 999999)), email))
    db.commit()
...
def change_password():
    ...
    if user:
        ...
        access_token = create_access_token(identity=email)
        data, query = register(
            user_info[0], new_password, email, access_token, "Mot de passe changé avec succès")
        cursor.execute(
            insert_query, (user_info[0], query["password"], email,  user_info[2], query["salt"], access_token, 0))
        db.commit()
        return jsonify(data)
    else:
        return jsonify({"success": False, "message": "Cet utilisateur n'existe pas. Inscrivez-vous!", "status": 404})
...
def add_posts():
    ...
    if email != "":
        if verify_user_exist(email=email):
            cursor = db.cursor()
            cursor.execute("INSERT INTO Posts (email, post) VALUES (%s, %s)", (
                email, post))
            db.commit()
    ...
    if verify_user_exist(email):
        ...
        if user_post and len(user_post) > 0:
            ...
            for post in user_post:
                posts["post"+str(i)] = {"id": post[0],
                                        "post": post[2], "created_at": post[6]}
                i += 1
...
def get_comments(post_id):
    ...
    if comments and len(comments) > 0:
        i = 1
        for comment in comments:
            all_comments["comment"+str(i)] = {"id": comment[0], "comment_email": comment[1],
                                              "comments": comment[3], "created_at": comment[4]}
            i += 1
